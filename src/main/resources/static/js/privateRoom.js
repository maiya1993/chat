...
function onConnected() {
    ...
    namePrivateRoom = window.location.pathname.split('/')[2];
    stompClient.subscribe('/topic/privateRoom/' + namePrivateRoom, onMessageReceived);

    // Tell your username to the server
    stompClient.send("/app/privateRoom/" + namePrivateRoom + "/addUser",
        {},
...
function sendMessage(event) {
    ...
    if(messageContent && stompClient) {
        var chatMessageForPrivateRoom = {
            sender: username,
            messageToUser: messageToUser,
            content: messageInput.value,
            type: 'CHAT'
        };
        stompClient.send("/app/privateRoom/" + namePrivateRoom + "/sendMessage", {}, JSON.stringify(chatMessageForPrivateRoom));
        messageInput.value = '';
    }
