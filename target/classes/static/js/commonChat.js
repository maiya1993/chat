...
function onConnected() {
    ...
    stompClient.subscribe('/topic/commonChat', onMessageReceived);

    // Tell your username to the server
    stompClient.send("/app/commonChat.addUser",
        {},
...
function sendMessage(event) {
    ...
    if(messageContent && stompClient) {
        var commonChatMessage = {
            sender: username,
            messageToUser: messageToUser,
            content: messageInput.value,
            type: 'CHAT'
        };
        stompClient.send("/app/commonChat.sendMessage", {}, JSON.stringify(commonChatMessage));
        messageInput.value = '';
    }
